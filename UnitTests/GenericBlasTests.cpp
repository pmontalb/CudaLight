...
namespace clt
{
	...
	static cl::gblas::mat GetInvertibleMatrix(unsigned nRows, const unsigned seed = 1234)
	{
		cl::gblas::mat A = cl::gblas::mat::RandomUniform(nRows, nRows, seed);
		auto _A = A.Get();
	...
	TEST_F(GenericBlasTests, AddMatrix)
	{
		cl::gblas::mat m1 = cl::gblas::mat::LinSpace(-1.0f, 1.0f, 100, 100);

		auto _m1 = m1.Get();

		cl::gblas::mat m2 = cl::gblas::mat::RandomUniform(m1.nRows(), m1.nCols(), 1234);
	...
	TEST_F(GenericBlasTests, BroadcastAdd)
	{
		cl::gblas::mat m1 = cl::gblas::mat::LinSpace(-1.0f, 1.0f, 64, 128);

		auto _m1 = m1.Get();
		auto m1Copy = m1;

		cl::gblas::vec v1 = cl::gblas::vec::RandomUniform(m1.nRows(), 1234);

		auto _v1 = v1.Get();

		cl::gblas::vec v2 = cl::gblas::vec::RandomUniform(m1.nCols(), 1234);
	...
	TEST_F(GenericBlasTests, ScaleColumns)
	{
		cl::gblas::mat m = cl::gblas::mat::RandomUniform(10, 100, 1234);
		cl::gblas::vec v(m.nCols(), 2.0);
	...
	TEST_F(GenericBlasTests, ElementWiseProduct)
	{
		cl::gblas::vec v1 = cl::gblas::vec::LinSpace(-1.0, 1.0, 100);
		auto _v1 = v1.Get();
	...
	TEST_F(GenericBlasTests, Multiply)
	{
		cl::gblas::mat m1(10, 10, 1.2345f);

		auto _m1 = m1.Get();

		cl::gblas::mat m2(10, 10, 9.8765f);
	...
	TEST_F(GenericBlasTests, SubMultiply)
	{
		...
		cl::gblas::mat m3(m1.nRows(), m2.nCols(), -123456789.0f);
		auto _initialM3 = m3.Get();

		cl::gblas::mat m4 = m1 * m2;
		auto _m4 = m4.Get();
	...
	TEST_F(GenericBlasTests, Dot)
	{
		cl::gblas::mat m1(10, 10, 1.2345f);

		auto _m1 = m1.Get();

		cl::gblas::vec v1(10, 9.8765f);
	...
	TEST_F(GenericBlasTests, Invert)
	{
		cl::gblas::mat v = GetInvertibleMatrix(128);

		cl::gblas::mat vMinus1(v);
		vMinus1.Invert();
	...
	TEST_F(GenericBlasTests, SolveLu)
	{
		cl::gblas::mat v = GetInvertibleMatrix(128);

		auto _v = v.Get();

		cl::gblas::mat u = GetInvertibleMatrix(v.nRows(), 2345);
		auto _u = u.Get();
	...
	TEST_F(GenericBlasTests, SolveQr)
	{
		cl::gblas::mat v = GetInvertibleMatrix(128);
	...
	TEST_F(GenericBlasTests, KroneckerProduct)
	{
		cl::gblas::vec u(64, 0.1f);

		auto _u = u.Get();

		cl::gblas::vec v(128, 0.2f);

		auto _v = v.Get();

		cl::gblas::mat A = cl::gblas::mat::KroneckerProduct(u, v, 2.0);
	...
	TEST_F(GenericBlasTests, RowWiseSum)
	{
		cl::gblas::mat A(128, 64);
	...
	TEST_F(GenericBlasTests, ColumnWiseSum)
	{
		cl::gblas::mat A(128, 64);
		A.RandomGaussian(1234);
	...
	TEST_F(GenericBlasTests, CubeWiseSum)
	{
		...
		{
			cl::gblas::mat out(T.nRows(), T.nCols(), 0.0);
			T.CubeWiseSum(out);
	...
	TEST_F(GenericBlasTests, BatchedKroneckerProduct)
	{
		unsigned nCubes = 64;

		cl::gblas::mat u(128, nCubes, 1.0);
		u.RandomUniform();

		auto _u = u.Get();

		cl::gblas::mat v(32, nCubes, 2.0);
		v.RandomGaussian();

		auto _v = v.Get();

		cl::gblas::ten A = cl::gblas::ten::KroneckerProduct(u, v, 1.0);
	...
	TEST_F(GenericBlasTests, ColumnWiseAbsoluteMinMax)
	{
		cl::gblas::mat A = cl::gblas::mat::LinSpace(-1.0f, 1.0f, 32, 128);
		auto _A = A.Get();
	...
	TEST_F(GenericBlasTests, CountEquals)
	{
		cl::gblas::vec u(64, 0.1f);
		cl::gblas::vec v(u);
		...
		cl::gblas::vec w = cl::gblas::vec::LinSpace(-3.14f, 3.14f, u.size());
