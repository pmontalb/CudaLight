...
namespace clt
{
	class OpenBlasTests : public ::testing::Test
	{
	};

	static cl::oblas::mat GetInvertibleMatrix(unsigned nRows, const unsigned seed = 1234)
	{
		cl::oblas::mat A = cl::oblas::mat::RandomUniform(nRows, nRows, seed);
		auto _A = A.Get();
	...
	TEST_F(OpenBlasTests, Add)
	{
		cl::oblas::vec v1 = cl::oblas::vec::LinSpace(-1.0, 1.0, 100);
		
		auto _v1 = v1.Get();

		cl::oblas::vec v2 = cl::oblas::vec::RandomUniform(v1.size(), 1234);
		...
		cl::oblas::ivec v5(32, 5);
		
		auto _v5 = v5.Get();

		cl::oblas::ivec v6(32, 7);
	...
	TEST_F(OpenBlasTests, AddMatrix)
	{
		cl::oblas::mat m1 = cl::oblas::mat::LinSpace(-1.0f, 1.0f, 100, 100);

		auto _m1 = m1.Get();

		cl::oblas::mat m2 = cl::oblas::mat::RandomUniform(m1.nRows(), m1.nCols(), 1234);
	...
	TEST_F(OpenBlasTests, BroadcastAdd)
	{
		cl::oblas::mat m1 = cl::oblas::mat::LinSpace(-1.0f, 1.0f, 64, 128);

		auto _m1 = m1.Get();
		auto m1Copy = m1;

		cl::oblas::vec v1 = cl::oblas::vec::RandomUniform(m1.nRows(), 1234);

		auto _v1 = v1.Get();

		cl::oblas::vec v2 = cl::oblas::vec::RandomUniform(m1.nCols(), 1234);
	...
	TEST_F(OpenBlasTests, Reciprocal)
	{
		cl::oblas::vec v1 = cl::oblas::vec::LinSpace(1.0, 2.0, 100);
	...
	TEST_F(OpenBlasTests, ScaleColumns)
	{
		cl::oblas::mat m = cl::oblas::mat::RandomUniform(10, 100, 1234);
		cl::oblas::vec v(m.nCols(), 2.0);
	...
	TEST_F(OpenBlasTests, ElementWiseProduct)
	{
		cl::oblas::vec v1 = cl::oblas::vec::LinSpace(-1.0, 1.0, 100);
		auto _v1 = v1.Get();
	...
	TEST_F(OpenBlasTests, Multiply)
	{
		cl::oblas::mat m1(10, 10, 1.2345f);

		auto _m1 = m1.Get();

		cl::oblas::mat m2(10, 10, 9.8765f);
	...
	TEST_F(OpenBlasTests, SubMultiply)
	{
		cl::oblas::mat m1(10, 10, 1.2345f);
		...
		cl::oblas::mat m3(m1.nRows(), m2.nCols(), -123456789.0f);
		auto _initialM3 = m3.Get();

		cl::oblas::mat m4 = m1 * m2;
		auto _m4 = m4.Get();
	...
	TEST_F(OpenBlasTests, Dot)
	{
		cl::oblas::mat m1(10, 10, 1.2345f);

		auto _m1 = m1.Get();

		cl::oblas::vec v1(10, 9.8765f);
	...
	TEST_F(OpenBlasTests, Invert)
	{
		cl::oblas::mat v = GetInvertibleMatrix(128);

		cl::oblas::mat vMinus1(v);
		vMinus1.Invert();
	...
	TEST_F(OpenBlasTests, SolveLu)
	{
		cl::oblas::mat v = GetInvertibleMatrix(128);

		auto _v = v.Get();

		cl::oblas::mat u = GetInvertibleMatrix(v.nRows(), 2345);
		auto _u = u.Get();
	...
	TEST_F(OpenBlasTests, KroneckerProduct)
	{
		cl::oblas::vec u(64, 0.1f);

		auto _u = u.Get();

		cl::oblas::vec v(128, 0.2f);

		auto _v = v.Get();

		cl::oblas::mat A = cl::oblas::mat::KroneckerProduct(u, v, 2.0);
	...
	TEST_F(OpenBlasTests, RowWiseSum)
	{
		cl::oblas::mat A(128, 64);
	...
	TEST_F(OpenBlasTests, ColumnWiseSum)
	{
		cl::oblas::mat A(128, 64);
		A.RandomGaussian(1234);
	...
	TEST_F(OpenBlasTests, CubeWiseSum)
	{
		cl::oblas::ten T(64, 128, 32);
		...
		{
			cl::oblas::mat out(T.nRows(), T.nCols(), 0.0);
			T.CubeWiseSum(out);
	...
	TEST_F(OpenBlasTests, BatchedKroneckerProduct)
	{
		unsigned nCubes = 64;

		cl::oblas::mat u(128, nCubes, 1.0);
		u.RandomUniform();

		auto _u = u.Get();

		cl::oblas::mat v(32, nCubes, 2.0);
		v.RandomGaussian();

		auto _v = v.Get();

		cl::oblas::ten A = cl::oblas::ten::KroneckerProduct(u, v, 1.0);
	...
	TEST_F(OpenBlasTests, ColumnWiseAbsoluteMinMax)
	{
		cl::oblas::mat A = cl::oblas::mat::LinSpace(-1.0f, 1.0f, 32, 128);
		auto _A = A.Get();
	...
	TEST_F(OpenBlasTests, CountEquals)
	{
		cl::oblas::vec u(64, 0.1f);
		cl::oblas::vec v(u);
		...
		cl::oblas::vec w = cl::oblas::vec::LinSpace(-3.14f, 3.14f, u.size());
		ASSERT_EQ(w.CountEquals(w), w.size());
		ASSERT_EQ(w.CountEquals(u), 0);
		ASSERT_EQ(w.CountEquals(v), 0);
	}
	
	TEST_F(OpenBlasTests, TransposeMultiply)
	{
		cl::oblas::mat A(64, 128);
		A.RandomUniform();

		cl::oblas::mat B(64, 32);  // for A^T * B
		B.RandomUniform();

		cl::oblas::mat C(16, 128);  // for A * C^T
		C.RandomUniform();

		cl::oblas::mat D(32, 64);  // for A^T * D^T
		D.RandomUniform();
		...
		for (size_t i = 0; i < A.nCols(); ++i)
		{
			for (size_t j = 0; j < D.nRows(); ++j)
			{
				...
				ASSERT_NEAR(goldenATDT / static_cast<double>(_ATDT[i + j * ATDT.nRows()]), 1.0, 5.5e-7);
			}
		}
	}
}
