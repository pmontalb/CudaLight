...
namespace clt
{
	class GenericBlasExtraRoutinesTests: public ::testing::Test
	{
	};

	TEST_F(GenericBlasExtraRoutinesTests, Sum)
	{
		const size_t size = 1024;
		auto v1 = cl::gblas::dvec::LinSpace(-1.0, 1.0, size);
		auto _v1 = v1.Get();
		...
		auto v2 = cl::gblas::vec::LinSpace(-1.0, 1.0, size + 1);
		auto _v2 = v2.Get();
	...
	TEST_F(GenericBlasExtraRoutinesTests, AbsoluteMinMax)
	{
		auto x = cl::gblas::vec::LinSpace(-1.0f, 1.0f, 128);
		auto _x = x.Get();
	...
	TEST_F(GenericBlasExtraRoutinesTests, MinMax)
	{
		auto x = cl::gblas::vec::LinSpace(-1.0f, 1.0f, 128);
		auto _x = x.Get();
