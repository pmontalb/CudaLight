...
namespace cl
{
	...
	{
	public:
		explicit TensorCollection(const std::vector<std::tuple<size_t, size_t, size_t>>& sizes_) : data(static_cast<unsigned>(std::accumulate(sizes_.begin(), sizes_.end(), 0ul, [](const auto& x, const auto& y) { return x + std::get<0>(y) * std::get<1>(y) * std::get<2>(y); })), 0.0), sizes(sizes_)
		{
