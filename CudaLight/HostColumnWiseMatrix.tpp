...
namespace cl { namespace host
{
	...
	ColumnWiseMatrix<ms, md>::ColumnWiseMatrix(const std::string& fileName, bool useMemoryMapping)
	{
		...
		cl::host::MatrixFromBinaryFile(m, nRows, nCols, fileName, useMemoryMapping);
	...
	{
		auto m = Get();
		cl::host::Print(m, nRows(), nCols(), label);
	}

	template<MemorySpace ms, MathDomain md>
	std::ostream& ColumnWiseMatrix<ms, md>::ToOutputStream(std::ostream& os) const
	{
		cl::host::MatrixToOutputStream(Get(), nRows(), nCols(), os);
		return os;
	}
	...
	{
		cl::host::MatrixToBinaryFile(Get(), nRows(), nCols(), fileName, false, compressed, mode);
	}
...
template<MemorySpace ms, MathDomain md>
	...
	{
		cl::host::MatrixToOutputStream(m.Get(), m.nRows(), m.nCols(), os);
	...
	ColumnWiseMatrix<ms, md> ColumnWiseMatrix<ms, md>::MatrixFromInputStream(std::istream& is)
	{
		...
		unsigned nRows = 0, nCols = 0;
		cl::host::MatrixFromInputStream(_mat, nRows, nCols, is);
	...
	ColumnWiseMatrix<ms, md> ColumnWiseMatrix<ms, md>::MatrixFromBinaryFile(const std::string& fileName, const bool compressed, const bool useMemoryMapping)
	{
		...
		unsigned nRows = 0, nCols = 0;
		cl::host::MatrixFromBinaryFile(_mat, nRows, nCols, fileName, false, compressed, useMemoryMapping);
	...
	void ColumnWiseMatrix<ms, md>::MatrixToBinaryFile(const ColumnWiseMatrix<ms, md>& m, const std::string& fileName, const bool tranposed, const bool compressed, const std::string mode)
	{
		...
		cl::host::MatrixToBinaryFile(_mat, m.nRows(), m.nCols(), fileName, tranposed, compressed, mode);
	}
