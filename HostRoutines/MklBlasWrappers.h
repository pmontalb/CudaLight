...
#include <array>
	...
	namespace cl { namespace routines { namespace mkr {
		...
		{
			mkl::MKL_Somatadd(columnMajorOrdering, mklOperation[static_cast<unsigned>(aOperation)], mklOperation[static_cast<unsigned>(bOperation)],
							  A.nRows, A.nCols,
							  static_cast<float>(alpha),
							  reinterpret_cast<float*>(A.pointer), A.leadingDimension,
							  static_cast<float>(beta),
							  reinterpret_cast<float*>(B.pointer), B.leadingDimension,
							  reinterpret_cast<float*>(A.pointer), A.leadingDimension);
		}
		template<>
		...
		{
			mkl::MKL_Domatadd(columnMajorOrdering, mklOperation[static_cast<unsigned>(aOperation)], mklOperation[static_cast<unsigned>(bOperation)],
							  A.nRows, A.nCols,
							  alpha,
							  reinterpret_cast<double*>(A.pointer), A.leadingDimension,
							  beta,
							  reinterpret_cast<double*>(B.pointer), B.leadingDimension,
							  reinterpret_cast<double*>(A.pointer), A.leadingDimension);
		}
		...
		{
			mkl::cblas_sgemm(columnMajorLayout, mklOperationsEnum[static_cast<unsigned>(bOperation)], mklOperationsEnum[static_cast<unsigned>(cOperation)],
		...
		{
			mkl::cblas_dgemm(columnMajorLayout, mklOperationsEnum[static_cast<unsigned>(bOperation)], mklOperationsEnum[static_cast<unsigned>(cOperation)],
							 static_cast<int>(nRowsB), static_cast<int>(nColsC), static_cast<int>(nColsB),
							 alpha,
		...
		inline void BatchedMultiply<MathDomain::Float>(MemoryCube& A, const MemoryCube& B, const MemoryCube& C, const unsigned strideB, const unsigned strideC, const MatrixOperation bOperation, const MatrixOperation cOperation, const double alpha, const double beta)
		{
			...
			mkl::cblas_sgemm_batch(columnMajorLayout, bOperations.data(), cOperations.data(),
								   nRowsA.data(), nColsA.data(), nColsB.data(),
								   alphas.data(),
								   bPointers.data(), ldb.data(),
								   cPointers.data(), ldc.data(),
								   betas.data(),
								   aPointers.data(), lda.data(),
								   static_cast<int>(nGroups), groupSizes.data());
		}
		template<>
		inline void BatchedMultiply<MathDomain::Double>(MemoryCube& A, const MemoryCube& B, const MemoryCube& C, const unsigned strideB, const unsigned strideC, const MatrixOperation bOperation, const MatrixOperation cOperation, const double alpha, const double beta)
		{
			...
			mkl::cblas_dgemm_batch(columnMajorLayout, bOperations.data(), cOperations.data(),
								   nRowsA.data(), nColsA.data(), nColsB.data(),
								   alphas.data(),
								   bPointers.data(), ldb.data(),
								   cPointers.data(), ldc.data(),
		...
		{
			mkl::cblas_sgemv(columnMajorLayout, mklOperationsEnum[static_cast<unsigned>(aOperation)],
		...
		{
			mkl::cblas_dgemv(columnMajorLayout, mklOperationsEnum[static_cast<unsigned>(aOperation)],
							 static_cast<int>(A.nRows), static_cast<int>(A.nCols),
							 alpha,
							 reinterpret_cast<double*>(A.pointer), static_cast<int>(A.leadingDimension),
		...
		{
			mkl::cblas_sger(columnMajorLayout, static_cast<int>(x.size), static_cast<int>(y.size),
							static_cast<float>(alpha),
							reinterpret_cast<float*>(x.pointer), 1,
							reinterpret_cast<float*>(y.pointer), 1,
		...
		{
			mkl::cblas_dger(columnMajorLayout, static_cast<int>(x.size), static_cast<int>(y.size),
							alpha,
							reinterpret_cast<double*>(x.pointer), 1,
							reinterpret_cast<double*>(y.pointer), 1,
